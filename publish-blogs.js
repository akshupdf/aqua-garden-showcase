/**
 * Script to create and publish two blog posts
 * 1. Delayed Monsoon Rains & Hydroponic Solutions
 * 2. Year-Round Hydroponics: 12 Months of Fresh Produce
 */
...
// Blog Post 1: Delayed Monsoon Rains
const delayedRainBlog = {
  title: "Delayed Monsoon Rains: Why Hydroponics is the Future of Farming",
  slug: "delayed-monsoon-rains-hydroponics-future-farming",
...
const yearRoundBlog = {
  ...
  slug: "year-round-hydroponics-12-months-fresh-harvest",
  ...
  category: "basics",
  is_published: true,
  cover_image: "https://images.unsplash.com/photo-1585320806297-9796583766ae?w=1200&h=600&fit=crop",
  content: [
    {
      id: "1",
      type: "heading",
...
async function publishBlogs() {
  try {
    ...
    // Publish first blog
    console.log('Creating blog: "Delayed Monsoon Rains..."');
    const { data: blog1, error: error1 } = await supabaseAdmin
      .from('blogs')
      .insert([{
        ...delayedRainBlog,
    ...
    if (error1) {
      console.error('Error creating first blog:', error1);
    } else {
      console.log('✓ First blog published successfully:', blog1[0].title);
    }
    ...
    // Publish second blog
    console.log('Creating blog: "12 Months of Fresh Harvest..."');
    const { data: blog2, error: error2 } = await supabaseAdmin
      .from('blogs')
      .insert([{
        ...yearRoundBlog,
        published_at: new Date().toISOString(),
    ...
    if (error2) {
      console.error('Error creating second blog:', error2);
    } else {
      console.log('✓ Second blog published successfully:', blog2[0].title);
    }

    console.log('\n✓ All blog posts published successfully!');
